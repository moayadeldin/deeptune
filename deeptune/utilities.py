...
from deeptune.options import make_parser, DEEPTUNE_RESULTS
...
def split_save_load_dataset(mode,type,input_dir, train_size, val_size, test_size, train_dataset_path,val_dataset_path, test_dataset_path, seed, batch_size,tokenizer,siglip=False):
    ...
    if type == 'text':
        ...
        if mode == 'train':
            ...
            train_dataset = TextDataset.from_parquet(parquet_file=train_dataset_path, tokenizer=tokenizer, max_length=512)
            val_dataset = TextDataset.from_parquet(parquet_file=val_dataset_path, tokenizer=tokenizer, max_length=512)
