...
class ParquetImageDataset(Dataset):
    ...
    def __init__(self, df: DataFrame, transform=None, processor=None):
        ...
        # self.image_bytes = self.data['image'].values
        # self.has_labels = 'target' in df.columns
        # self.labels = self.data['target'].values if self.has_labels else None
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor(self.labels[idx], dtype=torch.long) if self.has_labels else torch.tensor(0, dtype=torch.int)
