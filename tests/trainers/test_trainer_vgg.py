...
def test_trainer_step_classification():
    model = adjustedVGGNet(num_classes=3, vgg_net_version="vgg11", added_layers=1, task_type="cls")
...
def test_trainer_peft_step_classification():
    model = adjustedPeftVGGNet(num_classes=3, vgg_net_version="vgg11", added_layers=1, task_type="cls")
    train_loader = create_dummy_loader()
    val_loader = create_dummy_loader()
