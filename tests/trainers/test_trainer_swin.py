...
def test_trainer_step_classification():
    model = adjustedSwin(num_classes=3, swin_version="swin_t", added_layers=1, task_type="cls")
...
def test_trainer_peft_step_classification():
    model = adjustedPeftSwin(num_classes=3, swin_version="swin_s", added_layers=1, task_type="cls")
    train_loader = create_dummy_loader()
    val_loader = create_dummy_loader()
