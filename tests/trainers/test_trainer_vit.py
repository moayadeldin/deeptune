...
def test_trainer_step_classification():
    model = adjustedViT(num_classes=3, vit_version="dummy", added_layers=1, task_type="cls")
...
def test_trainer_peft_step_classification():
    model = adjustedViTPeft(num_classes=3, vit_version="dummy", added_layers=1, task_type="cls")
    train_loader = create_dummy_loader()
    val_loader = create_dummy_loader()
